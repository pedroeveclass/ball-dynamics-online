-- Luva de Inverno picks "long sleeve" (full GK arm) or "short sleeve" (just
-- the glove on a bare arm) at equip time — not a left/right side. The same
-- `side` column on store_purchases now also stores those values.
--
-- equip_store_item is updated to accept the new values; the constraint is
-- widened so the row insert is valid.
...
ALTER TABLE public.store_purchases
  ...
  CHECK (side IS NULL OR side IN ('left', 'right', 'long', 'short'));
...
BEGIN
  ...
  v_side := CASE WHEN p_side IN ('left', 'right', 'long', 'short') THEN p_side ELSE NULL END;
