-- ═══════════════════════════════════════════════════════════
-- Move the R$1.000.000 "novo jogador" charge inside the RPC
-- so the debit is atomic with the new player creation.
--
-- Before: PlayerProfilePage debited the active player via a
-- plain UPDATE before redirecting to /onboarding/player. If the
-- user closed the tab in onboarding, they lost R$1M with no
-- player to show for it. Now the RPC checks balance, locks the
-- charge source FOR UPDATE, creates the new profile, and debits
-- in the same transaction.
--
-- Charge source: profiles.active_player_profile_id when set;
-- fallback to the oldest player_profiles row owned by the user.
-- First-time onboarding (no existing profile) is free, same as
-- before.
-- ═══════════════════════════════════════════════════════════
