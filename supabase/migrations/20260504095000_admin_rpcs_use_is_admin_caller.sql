-- ─────────────────────────────────────────────────────────────
-- Hotfix: admin RPCs must use is_admin_caller(), not is_admin()
-- ─────────────────────────────────────────────────────────────
-- The 20260504080000 batch referenced public.is_admin(auth.uid()), a
-- function that doesn't exist. PL/pgSQL doesn't validate function
-- bodies at CREATE time, so the migration succeeded but every call
-- raised 42883 at runtime. Re-emit each one against the canonical
-- helper public.is_admin_caller() (no args, reads auth.uid()).
-- Same hotfix applies to the admin SELECT policy on store_purchases.
...
BEGIN
  ...
  IF v_year IS NULL THEN RETURN jsonb_build_object('ok', false, 'reason', 'no_open_season'); END IF;
...
BEGIN
  IF NOT public.is_admin_caller() THEN RAISE EXCEPTION 'admin only'; END IF;
  IF p_delta_days IS NULL OR p_delta_days = 0 THEN RETURN jsonb_build_object('ok', false, 'reason', 'no_delta'); END IF;
...
SET search_path = public
AS $$
DECLARE v_clubs INT; v_seasons INT;
BEGIN
  IF NOT public.is_admin_caller() THEN RAISE EXCEPTION 'admin only'; END IF;
...
BEGIN
  ...
  IF v_player IS NULL THEN RETURN jsonb_build_object('ok', false, 'reason', 'player_not_found'); END IF;
...
-- Admin SELECT policy on store_purchases (for the image-report image preview).
