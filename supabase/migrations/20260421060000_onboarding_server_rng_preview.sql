-- ═══════════════════════════════════════════════════════════
-- Onboarding: server-owned ±3 base jitter, seeded per user/attr.
--
-- Motivation: we want back the "each attribute rolls ±3" feel that
-- was removed in 20260413030000, but without re-introducing the old
-- client/server desync bug. The 2026-04-13 fix made base fully
-- deterministic (no jitter) because the client and server were each
-- rolling their own random values.
--
-- New design: the server is the only authority on base attributes.
--   • get_onboarding_preview(position, height, body_type) returns the
--     full attribute map the client should DISPLAY during distribution.
--   • create_player_profile(...) recomputes the same map with the same
--     seed (user_id + attr_key) and applies the 40 extra points on top.
--   • The client no longer computes base attributes locally — it only
--     shows what the preview RPC returned and tracks the 40 extras.
--
-- Jitter: int in [-3, +3] derived from hashtext(user_id || '|' || key).
-- Stable per (user, attribute) — if the user switches archetype /
-- height / position, the per-attribute roll stays the same, only the
-- boosts change. That was preferred over a per-(user+archetype+height+
-- position) seed (which would feel like a reroll when swapping).
--
-- Everything else is unchanged: post-boost clamp [10, 65], final
-- clamp [10, 70], extras sum = 40 and capped at 75 per attribute.
-- ═══════════════════════════════════════════════════════════

-- ── Shared helper: base + jitter + boosts, no extras ──
CREATE OR REPLACE FUNCTION public.compute_onboarding_base_attrs(
  p_user_id UUID,
...
DECLARE
  v_attrs JSONB := '{}';
  v_is_gk BOOLEAN := (p_primary_position = 'GK');
  v_base NUMERIC := 35;
  v_gk_base NUMERIC := CASE WHEN v_is_gk THEN 35 ELSE 12 END;
...
BEGIN
  ...
  -- ── Base + ±3 jitter seeded by (user_id, attr_key) ──
  -- hashtext returns int4; mod 7 gives [0,6], minus 3 → [-3,+3].
  FOREACH v_key IN ARRAY v_field_keys LOOP
    v_jitter := (abs(hashtext(p_user_id::text || '|' || v_key)) % 7) - 3;
    v_attrs := v_attrs || jsonb_build_object(v_key, v_base + v_jitter);
  END LOOP;
  FOREACH v_key IN ARRAY v_gk_keys LOOP
    v_jitter := (abs(hashtext(p_user_id::text || '|' || v_key)) % 7) - 3;
    v_attrs := v_attrs || jsonb_build_object(v_key, v_gk_base + v_jitter);
  END LOOP;

  -- ── Position boosts (clamp [10, 65]) ──
  v_pos_boosts := CASE p_primary_position
    ...
    WHEN 'LWB' THEN '{"velocidade":7,"aceleracao":6,"resistencia":7,"stamina":5,"posicionamento_defensivo":3,"marcacao":3,"drible":3,"posicionamento_ofensivo":3}'::JSONB
    WHEN 'RWB' THEN '{"velocidade":7,"aceleracao":6,"resistencia":7,"stamina":5,"posicionamento_defensivo":3,"marcacao":3,"drible":3,"posicionamento_ofensivo":3}'::JSONB
    WHEN 'DM' THEN '{"marcacao":6,"desarme":8,"posicionamento_defensivo":8,"antecipacao":6,"trabalho_equipe":4}'::JSONB
    WHEN 'CDM' THEN '{"marcacao":6,"desarme":8,"posicionamento_defensivo":8,"antecipacao":6,"trabalho_equipe":4}'::JSONB
    WHEN 'CM' THEN '{"passe_baixo":6,"visao_jogo":4,"tomada_decisao":4,"trabalho_equipe":4,"resistencia":4}'::JSONB
    WHEN 'LM' THEN '{"velocidade":5,"resistencia":6,"passe_baixo":5,"drible":5,"posicionamento_ofensivo":3,"tomada_decisao":3}'::JSONB
    WHEN 'RM' THEN '{"velocidade":5,"resistencia":6,"passe_baixo":5,"drible":5,"posicionamento_ofensivo":3,"tomada_decisao":3}'::JSONB
    ...
    WHEN 'CF' THEN '{"acuracia_chute":6,"forca_chute":4,"posicionamento_ofensivo":8,"passe_baixo":4,"drible":4,"um_toque":4,"visao_jogo":3}'::JSONB
  ...
  -- ── Final clamp [10, 70] before extras ──
...
-- ── Onboarding preview RPC used by the client ──
CREATE OR REPLACE FUNCTION public.get_onboarding_preview(
  p_primary_position TEXT,
  p_height TEXT,
  p_body_type TEXT
)
...
BEGIN
  ...
  RETURN public.compute_onboarding_base_attrs(v_user_id, p_primary_position, p_height, p_body_type);
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_onboarding_preview(TEXT, TEXT, TEXT) TO authenticated;

-- ── Rewrite create_player_profile to delegate to the helper ──
...
BEGIN
  ...
  -- Same seeded base attributes the preview returned.
  v_attrs := public.compute_onboarding_base_attrs(v_user_id, p_primary_position, p_height, p_body_type);

  -- Apply user-distributed extras on top, capped at 75.
  IF p_extra_points IS NOT NULL THEN
    FOR v_key IN SELECT jsonb_object_keys(p_extra_points) LOOP
      v_extra := (p_extra_points ->> v_key)::INT;
      IF v_extra > 0 THEN
