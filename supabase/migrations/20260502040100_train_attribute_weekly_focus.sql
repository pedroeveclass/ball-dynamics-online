-- ═══════════════════════════════════════════════════════════
-- train_attribute + auto_train_attribute: read coach_weekly_boost
-- and add +10% gain when this week's boost is `training_focus` and
-- the attribute belongs to the chosen category.
--
-- Categories (canonical, mirror src/lib/attributes.ts ATTRIBUTE_CATEGORIES):
...
--   Mental   → visao_jogo, tomada_decisao, antecipacao, trabalho_equipe, coragem,
--              posicionamento_ofensivo, posicionamento_defensivo
--   Chute    → cabeceio, acuracia_chute, forca_chute
--   Goleiro  → reflexo, posicionamento_gol, defesa_aerea, pegada, saida_gol, um_contra_um,
--              distribuicao_curta, distribuicao_longa, tempo_reacao, comando_area
-- ═══════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.attribute_category(p_attr_key TEXT)
RETURNS TEXT
LANGUAGE sql IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_attr_key = ANY(ARRAY['velocidade','aceleracao','agilidade','forca','equilibrio','resistencia','pulo','stamina']) THEN 'Físico'
    WHEN p_attr_key = ANY(ARRAY['drible','controle_bola','marcacao','desarme','um_toque','curva','passe_baixo','passe_alto']) THEN 'Técnico'
    WHEN p_attr_key = ANY(ARRAY['visao_jogo','tomada_decisao','antecipacao','trabalho_equipe','coragem','posicionamento_ofensivo','posicionamento_defensivo']) THEN 'Mental'
    WHEN p_attr_key = ANY(ARRAY['cabeceio','acuracia_chute','forca_chute']) THEN 'Chute'
    WHEN p_attr_key = ANY(ARRAY['reflexo','posicionamento_gol','defesa_aerea','pegada','saida_gol','um_contra_um','distribuicao_curta','distribuicao_longa','tempo_reacao','comando_area']) THEN 'Goleiro'
    ELSE NULL
  END;
$$;

GRANT EXECUTE ON FUNCTION public.attribute_category(TEXT) TO authenticated, service_role;
...
BEGIN
  ...
  IF v_club_id IS NOT NULL THEN
    ...
    -- Weekly Coach Boost: training_focus +10% when category matches
    SELECT cwb.boost_type, cwb.boost_param INTO v_active_boost
      FROM public.coach_weekly_boost cwb
      WHERE cwb.club_id = v_club_id
        AND cwb.iso_week_start = public.current_brt_week_start()
      LIMIT 1;
    IF v_active_boost.boost_type = 'training_focus'
       AND v_active_boost.boost_param = public.attribute_category(p_attribute_key) THEN
      v_focus_bonus := 0.10;
    END IF;
  END IF;
  ...
  v_growth := v_growth * (1 + v_coach_bonus + v_tc_bonus + v_trainer_bonus + v_focus_bonus);
