-- ═══════════════════════════════════════════════════════════
-- Allow create_player_profile to be called when the user
-- already owns one or more player profiles.
--
-- Background: 20260328070000_multi_player.sql introduced the
-- 2-slot character system (profiles.active_player_profile_id +
-- dropped the unique index on player_profiles.user_id), but the
-- create_player_profile RPC was never updated. Successive
-- rewrites in 20260413030000 / 20260415010000 / 20260421060000
-- preserved the legacy guard:
--
--   IF EXISTS (SELECT 1 FROM player_profiles WHERE user_id = v_user_id) THEN
--     RAISE EXCEPTION 'User already has a player profile';
--
-- That makes "Criar Novo Jogador" on PlayerProfilePage fail
-- after the redirect to /onboarding/player. This migration
-- recreates the RPC verbatim from 20260421060000 minus that
-- guard. All other validation, attribute computation, and
...
BEGIN
  ...
  -- Multi-player slots: no longer reject users who already own a profile.
