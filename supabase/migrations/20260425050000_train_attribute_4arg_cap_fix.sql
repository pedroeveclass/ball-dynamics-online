-- ═══════════════════════════════════════════════════════════
-- Restore 4-arg get_attribute_cap calls in train_attribute /
-- auto_train_attribute.
--
-- Migration 20260424010000_training_pace_and_aging.sql recreated
-- both RPCs and silently reverted the 4-arg cap signature
-- introduced in 20260420060000_caps_by_position_refined.sql.
-- Result on prod: every train call raises
--   `function public.get_attribute_cap(text, text, text) does not exist`
-- and ST/Tower/Muito-Alto loses the position-aware cabeceio cap.
--
-- Fix-forward: re-emit both RPCs with the same body as
-- 20260424010000 (pace factor 0.40, retirement check, etc.)
-- but calling the 4-arg cap with primary_position.
-- ═══════════════════════════════════════════════════════════
