-- ═══════════════════════════════════════════════════════════
-- Phase 3 of notification i18n: migrate the store-gift insert
-- inside purchase_store_item to emit i18n_key='store_gift' with
-- {club, item} params alongside the legacy PT title/body.
--
-- Body is byte-for-byte identical to 20260420040000_cereal_bar_consumable.sql
-- except for the INSERT INTO notifications block.
-- ═══════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF p_buyer_type = 'club' AND v_player.user_id IS NOT NULL AND v_player.user_id != auth.uid() THEN
    ...
    VALUES (
      ...
      'store_gift',
      jsonb_build_object(
        'club', COALESCE(v_club_name, ''),
        'item', COALESCE(v_item.name_pt, v_item.name),
