...
const PAR2_PT: Record<Par2Type, string[]> = {
  hat_trick: [
    "{hat_trick_player} foi o nome inquestionável da partida — {hat_trick_goals} gols solitários, cada um construído de uma maneira diferente, cada um decisivo pro desenrolar do jogo. O jogador mostrou faro de gol em todas as situações que apareceram, finalizou com precisão dentro e fora da área, e provou por que tem chamado atenção dos olheiros das principais equipes. Hat-trick que entra direto pra galeria das melhores atuações individuais da temporada.",
    "Noite mágica de {hat_trick_player}: {hat_trick_goals} gols num só jogo, números de jogador em estado de graça absoluto. Não tem como elaborar sobre uma atuação dessas — o jogador esteve em todos os lances importantes, encontrou os caminhos do gol em situações distintas e carregou o time sozinho nos momentos em que precisou. Hat-trick que coloca o nome dele entre os destaques da rodada e prova mais uma vez seu valor pro elenco.",
  ...
  generic: [
    "{top_scorer} fez o gol que decidiu a partida numa jogada de oportunismo dentro da área. O jogador esteve no lugar certo na hora certa, finalizou sem hesitar e garantiu três pontos preciosos pro time. Não foi a atuação mais espetacular da temporada, mas teve a importância que precisava ter — pra times que disputam objetivos importantes, gols como esse fazem toda a diferença na soma final dos pontos.",
...
const PAR3_EN: Record<Par3Type, string[]> = {
  ...
  new_top_scorer: [
    "Another match standout: {season_top_scorer} now leads the competition's top scorer race with {season_top_scorer_goals} goals on the season. The goals from this match placed him ahead of his former chasers, and the player continues in an impressive productive phase, in the rhythm of the club's best goalscorer in recent history. A serious bid to compete for the championship's golden boot, especially if he maintains this level for the remaining rounds.",
    "Top scorer leadership changes hands: {season_top_scorer} reaches {season_top_scorer_goals} goals on the season and takes alone the first position among scorers. A scorer in state of grace, finding the net in sequence and carrying the team whenever they appear in front of goal. Whoever follows the championship knows he was pushing this number round after round — and now he is, officially, the competition's best finisher.",
    "{season_top_scorer} is the new isolated top scorer leader: {season_top_scorer_goals} goals on the season, protagonist place in the goalscorers' ranking. Another brilliant chapter in an individual campaign that was no longer being hidden — the player had been showing good form for weeks, and the goals in this match only confirmed the status. He promises to produce talk and scout attention in the coming rounds, which will be decisive in closing the season with the golden boot.",
  ],
  draw_neutral: [
