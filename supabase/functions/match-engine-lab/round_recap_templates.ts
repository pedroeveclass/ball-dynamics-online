...
const PAR2_PT: Record<Par2Type, string[]> = {
  ...
  top_scorer_round: [
    "{top_scorer}, do {top_scorer_club}, foi o artilheiro isolado da rodada com {top_scorer_goals} gols. O atacante esteve em todos os lances importantes do seu time, finalizou com qualidade técnica visível e se firma como referência ofensiva do elenco. Performance que confirma seu lugar entre os melhores goleadores da temporada e mantém a expectativa em alta pra próxima rodada que se aproxima.",
    "Artilharia da rodada teve nome certo: {top_scorer}, do {top_scorer_club}, com {top_scorer_goals} gols. O atacante segue numa fase produtiva impressionante, marcando em sequência e carregando o time sempre que aparece na frente da meta adversária. Tipo de regularidade que coloca jogadores entre os candidatos a chuteira de ouro do campeonato — e ele claramente está nessa briga.",
    "{top_scorer} foi o destaque ofensivo da rodada com {top_scorer_goals} gols pelo {top_scorer_club}. Mais um capítulo de uma temporada que vem ganhando contornos memoráveis pro atacante, que segue mostrando por que é peça fundamental no esquema do treinador. Quem acompanha o campeonato sabe que o jogador vinha empurrando essa cifra rodada após rodada — e agora consolidou a vaga entre os artilheiros.",
  ],
  generic_round: [
...
const PAR3_PT: Record<Par3Type, string[]> = {
  ...
  top_scorer_race: [
    "Outro destaque que se desenrola na competição: {season_top_scorer} agora lidera a artilharia da temporada com {season_top_scorer_goals} gols. Os números desta rodada o colocaram em posição privilegiada na briga pela chuteira de ouro, e o atacante segue numa fase produtiva impressionante. Promete render conversa nas próximas rodadas, com possíveis perseguidores tentando descontar a vantagem antes do fim do campeonato.",
  ...
  "{season_top_scorer} continua liderando a corrida pela artilharia da Liga: {season_top_scorer_goals} gols na temporada após esta rodada. O atacante vinha empurrando essa cifra rodada após rodada, e nesta semana confirmou de novo o status entre os melhores finalizadores. Ainda há rodadas pela frente pra brigarem pela chuteira de ouro, mas hoje ele claramente é o favorito e segue mostrando regularidade que assusta os perseguidores.",
  ],
  generic_table: [
...
const PAR2_EN: Record<Par2Type, string[]> = {
  ...
  top_scorer_round: [
    "{top_scorer}, of {top_scorer_club}, was the round's top scorer with {top_scorer_goals} goals. The striker was in every important play for his team, finished with visible technical quality, and cemented his position as offensive reference of the squad. A performance that confirms his place among the season's best goalscorers and keeps expectations high for the next round.",
    "The round's golden boot had a clear name: {top_scorer}, of {top_scorer_club}, with {top_scorer_goals} goals. The striker continues in an impressive productive phase, scoring in sequence and carrying the team whenever he appears in front of goal. The kind of consistency that places players among golden-boot candidates — and he's clearly in that race.",
    "{top_scorer} was the round's offensive standout with {top_scorer_goals} goals for {top_scorer_club}. Another chapter of a season taking memorable shape for the striker, who keeps showing why he's a fundamental piece in the coach's setup. Whoever follows the championship knows he was pushing this number round after round — and now consolidates a slot among the top scorers.",
  ],
  generic_round: [
