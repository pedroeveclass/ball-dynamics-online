...
export function ManagerReportsIntroTour({ enabled }: Props) {
  const { t } = useTranslation('tour');
  const { shouldRun, markSeen } = useLocalTour('manager_reports_intro');
  ...
  const steps: Step[] = [
    { target: '[data-tour="reports-summary"]', content: renderStep('manager_reports_intro.step1.title', 'manager_reports_intro.step1.body'), placement: 'bottom', skipBeacon: true },
    { target: '[data-tour="reports-list"]', content: renderStep('manager_reports_intro.step2.title', 'manager_reports_intro.step2.body'), placement: 'top', skipBeacon: true },
  ];
