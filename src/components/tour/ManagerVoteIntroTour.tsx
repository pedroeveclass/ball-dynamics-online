...
export function ManagerVoteIntroTour({ enabled }: Props) {
  const { t } = useTranslation('tour');
  const { shouldRun, markSeen } = useLocalTour('manager_vote_intro');
  ...
  const steps: Step[] = [
    { target: '[data-tour="vote-form"]', content: renderStep('manager_vote_intro.step1.title', 'manager_vote_intro.step1.body'), placement: 'bottom', skipBeacon: true },
    { target: '[data-tour="vote-results"]', content: renderStep('manager_vote_intro.step2.title', 'manager_vote_intro.step2.body'), placement: 'top', skipBeacon: true },
  ];
