...
export function ManagerSquadIntroTour({ enabled }: Props) {
  const { t } = useTranslation('tour');
  const { shouldRun, markSeen } = useLocalTour('manager_squad_intro');
  ...
  const steps: Step[] = [
    { target: '[data-tour="manager-squad-header"]', content: renderStep('manager_squad_intro.step1.title', 'manager_squad_intro.step1.body'), placement: 'bottom', skipBeacon: true },
    { target: '[data-tour="manager-squad-summary"]', content: renderStep('manager_squad_intro.step2.title', 'manager_squad_intro.step2.body'), placement: 'bottom', skipBeacon: true },
  ];
