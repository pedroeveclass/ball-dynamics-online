...
export function StoreManagerIntroTour({ enabled }: Props) {
  const { t } = useTranslation('tour');
  const { shouldRun, markSeen } = useLocalTour('store_intro_manager');
  ...
  const steps: Step[] = [
    { target: '[data-tour="store-tabs"]', content: renderStep('store_intro_manager.step1.title', 'store_intro_manager.step1.body'), placement: 'bottom', skipBeacon: true },
