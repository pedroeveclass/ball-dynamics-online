...
export function ManagerFacilitiesIntroTour({ enabled }: Props) {
  const { t } = useTranslation('tour');
  const { shouldRun, markSeen } = useLocalTour('manager_facilities_intro');
  ...
  const steps: Step[] = [
    { target: '[data-tour="facilities-summary"]', content: renderStep('manager_facilities_intro.step1.title', 'manager_facilities_intro.step1.body'), placement: 'bottom', skipBeacon: true },
    { target: '[data-tour="facilities-list"]', content: renderStep('manager_facilities_intro.step2.title', 'manager_facilities_intro.step2.body'), placement: 'top', skipBeacon: true },
  ];
