...
interface Props {
  scope: 'round_mvp' | 'season_mvp';
  entityId: string;
  voteRpc: 'vote_round_mvp' | 'vote_season_mvp';
  anchorId: string;
  title: ReactNode;
}

export function MvpPollCard({ scope, entityId, voteRpc, anchorId, title }: Props) {
  ...
  async function handleVote(candidateId: string) {
    ...
    const { error } = await supabase.rpc(voteRpc as any, {
      p_poll_id: poll.id,
  ...
  return (
    <div id={anchorId} className="stat-card space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
        {(poll.candidates ?? []).map((c) => {
          ...
          const ratingDisplay = c.avg_rating ?? c.rating;
              ...
              <div className="flex items-end justify-between">
                <div className="flex items-center gap-0.5">
                  ...
                  <span className="text-xs font-display font-bold">
                    {ratingDisplay != null ? Number(ratingDisplay).toFixed(1) : '—'}
                  </span>
                  {c.matches != null && (
